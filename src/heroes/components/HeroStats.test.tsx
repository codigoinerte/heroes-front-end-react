...
const renderHeroStats = (mockData?:Partial<SummaryInformationResponse>) => {
    ...
    if(mockData){
        mockUseHeroSummary.mockReturnValue({ data: mockData } as unknown as ReturnType<typeof useHeroSummary>)
    }else{
        mockUseHeroSummary.mockReturnValue({ data: undefined } as unknown as ReturnType<typeof useHeroSummary>)
    }
...
describe('HeroStats', ()=> {
    ...
    test('should change the percentage of favorites when a hero is added to favorites', () => { 
        ...
        const favoriteCounteElement = screen.getByTestId("favorite-count");
        expect(favoritePercentageElement.innerHTML).toContain("4.00");
        expect(favoriteCounteElement.innerHTML).toContain("1");
